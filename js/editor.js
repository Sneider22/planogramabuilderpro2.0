...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (headerStoreName && activeStore) {
        headerStoreName.innerText = activeStore.name;
    }
    ...
    const canDropProduct = (shelfIndex, productId) => {
        ...
        if (shelf.type === 'perchero') {
            ...
            const numHooks = Math.floor(state.gondola.width / spacing);
            const margin = (state.gondola.width - (numHooks - 1) * spacing) / 2;
        ...
        return (usedWidth + product.width) <= state.gondola.width;
    };
    ...
    function renderGondola() {
        ...
        g.shelves.forEach((shelf, idx) => {
            ...
            shelf.products.forEach(p => {
                ...
                if (maxDepth > g.shelfDepth) hasDepthCollision = true;
            });
        ...
        [...g.shelves].reverse().forEach(shelf => {
            ...
            if (shelf.type === 'perchero') {
                ...
                slider.addEventListener('input', (e) => {
                    sliderLabel.innerText = `Separación: ${e.target.value}cm`;
                });
